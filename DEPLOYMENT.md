...
This repo is **deployment-ready, not deployed**. No live Cloudflare account, GitHub remote, domain, or Resend account was available during this build (see README.md for why — a placeholder business identity was used throughout). Everything below is written so a competent developer with real accounts can go from this repo to a live site in well under 30 minutes.

## 0. Prerequisites

- A Cloudflare account (free tier is sufficient — see INFRASTRUCTURE.md for exactly what that covers).
- A domain you control (for DNS). Replace every `nirmaanwebworks.in` / `nirmaan-webworks` reference in `config/site.ts`, `wrangler.jsonc`, `wrangler.retry.jsonc`, and `astro.config.mjs`'s `site` field with your real domain/business name **first** — `npm run build` will fail loudly (see `scripts/validate-release.mjs`) if `config/site.ts`'s business fields are malformed or production secrets are missing, but it cannot detect "still a placeholder," so review `config/site.ts` by hand before deploying for real.
- Node.js 20+, npm.
...
```
npx wrangler d1 create nirmaan_leads
```
...
Verify its Cron Trigger is active: Cloudflare dashboard → Workers & Pages → `nirmaan-webworks-retry` → Triggers. It should show `*/30 * * * *`.
